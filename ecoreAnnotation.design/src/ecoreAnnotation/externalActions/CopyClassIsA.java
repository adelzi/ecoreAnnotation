...
public class CopyClassIsA implements IExternalJavaAction {
	...
	tmp.setName(classes.get(0).getName());
	/*
	for(EAttribute attr: classes.get(0).getEAllAttributes()){
		tmp.getEStructuralFeatures().add(EcoreUtil.copy(attr));
	}*/
	tmp.getEStructuralFeatures().addAll(EcoreUtil.copyAll(classes.get(0).getEAllAttributes()));
	//tmp.
